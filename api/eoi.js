...
export default async function handler(req, res) {
  ...
  // Honeypot: bots complete this invisible field; treat it as a successful no-op.
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res.status(502).json({ ok: false, error: 'The email service rejected the submission.' });
    }
